...
async function joinRoomById(roomId) {
    ...
    if (roomSnapshot.exists) {
        ...
        document.querySelector(
            '#currentRoom').innerText = `Videochat: ${roomId} - bestaat niet (meer)`;
    }
}
...
function init() {
    ...
    const localVideoScreen = document.getElementById("localVideo")

    openVideoChat.addEventListener("click", () => {
        openUserMedia()
        videosScreen.style.display = "flex"
        buttons.style.display = "flex"
        buttonsRoom.style.display = "flex"
        localVideoScreen.style.display = "block"
        localVideoScreen.scrollIntoView()
    });
